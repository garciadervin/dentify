...
export async function initLocalDB(): Promise<void> {
  try {
    // expo-sqlite is native-only; the dynamic require keeps this module
    // importable on web, where conversations.web.ts is used instead.
    ...
    console.warn('conversations: failed to init local DB', e);
    db = null;
  }
}
...
export async function saveConversationLocal(conv: Conversation): Promise<void> {
  ...
  try {
    ...
    console.warn('conversations: failed to save conversation locally', e);
  }
}
...
export async function getConversationsLocal(): Promise<Conversation[]> {
  ...
  try {
    ...
    return (rows as Record<string, unknown>[]).map((r) => ({
      ...
      messages: JSON.parse(String(r.messages)) as StoredMessage[],
      started_at: String(r.started_at),
      last_updated: String(r.last_updated),
    }));
  } catch (e) {
    console.warn('conversations: failed to get conversations locally', e);
    return [];
  }
}
...
export async function deleteConversationLocal(id: string): Promise<void> {
  ...
  try {
    ...
    console.warn('conversations: failed to delete conversation', e);
  }
}
