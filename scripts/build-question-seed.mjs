...
function toRow(q) {
  ...
  if (t === 'case') {
    ...
    caseText = sqlStr(q.caseText);
  }

  const points = q.points ?? defaultPoints(q);
  return `('${q.specialty_slug}', ${q.level}, ${sqlStr(t)}, ${sqlStr(q.question)}, ${options}, ${correctIndex}, ${correctIndexes}, ${pairs}, ${orderItems}, ${caseId}, ${caseText}, ${q.hint ? sqlStr(q.hint) : 'NULL'}, ${points}, ${q.level}, ${sqlTextArray(q.tags)})`;
}
...
for (let i = 0; i < allRows.length; i += CHUNK) {
  ...
  inserts.push(`insert into public.questions (specialty_slug, level, question_type, question, options, correct_index, correct_indexes, pairs, order_items, case_id, case_text, hint, points, difficulty, tags)
  values
  ${chunk};`);
}
